 ...
 * @brief Differential drive motor controller implementation for dual
 * parallel-wired L298N drivers.
 */
...
void driveSide(int8_t v, int ena_pin, int in1_pin, int in2_pin,
               int pwm_channel) {
  (void)ena_pin; // Pin is attached to pwm_channel during initialization
  uint32_t duty = 0;

  if (v > 0) {
    // Forward: in1_pin = LOW, in2_pin = HIGH
    ...
    // Reverse: in1_pin = HIGH, in2_pin = LOW
    // Hard safety rule, non-negotiable: write the pin turning OFF to LOW
    // BEFORE writing the pin turning ON to HIGH.
...
void motor_driver_stop(void) { driveMotors(0, 0); }
